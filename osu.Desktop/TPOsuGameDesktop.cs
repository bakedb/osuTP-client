...
{
    internal partial class TPOsuGameDesktop : TPOsuGame
    {
        ...
        public TPOsuGameDesktop(string[]? args = null)
            : base(args)
        {
        }
        ...
        {
            // If this is first time we've run game, ie it is being installed,
            // reset user's release stream to "lazer".
            //
            // This ensures that if a user is trying to recover from a failed startup on an unstable release stream,
            // game doesn't immediately try and update them back to release stream after starting up.
            if (IsFirstRun)
